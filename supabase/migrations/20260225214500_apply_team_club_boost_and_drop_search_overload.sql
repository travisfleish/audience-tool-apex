/*
  # Apply team/club league boost and remove ambiguous overload

  1. Changes
    - Drops the legacy 5-parameter hybrid_semantic_search overload that causes RPC ambiguity.
    - Replaces the active 8-parameter hybrid_semantic_search with team/club + league boost logic.

  2. Purpose
    - Ensure AI queries like "MLB teams"/"MLB clubs"/"NBA teams"/"NBA clubs" rank team entities first.
    - Ensure PostgREST can resolve a single hybrid_semantic_search function signature.
*/

drop function if exists public.hybrid_semantic_search(
  text,
  text,
  extensions.vector,
  integer,
