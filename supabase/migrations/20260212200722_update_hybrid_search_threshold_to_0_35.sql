/*
  # Update Default Score Threshold to 0.35

  1. Changes
    - Updates both hybrid_semantic_search overloads (4-param and 5-param) to use 0.35 as default score_threshold
    - Previously was 0.15, which allowed too much noise through

  2. Rationale
    - Testing showed results below 0.35 are consistently off-topic or tangentially related
    - 0.35 preserves all strong and good matches while cutting the noise floor
    - Basketball: keeps 89+ relevant results, cuts 382 noisy ones
    - Youth sports: keeps all 23 Youth Sports audiences, cuts unrelated demographics
*/
...
CREATE OR REPLACE FUNCTION public.hybrid_semantic_search(
  ...
  score_threshold double precision DEFAULT 0.35
)
RETURNS TABLE(
  id uuid,
  name text,
...
-- 5-parameter overload (with raw_query_text and exact match boost)
CREATE OR REPLACE FUNCTION public.hybrid_semantic_search(
  query_text text,
  raw_query_text text,
  query_embedding vector,
